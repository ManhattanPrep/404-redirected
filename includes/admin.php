...
function wbz404_getTableOptions() {
  ...
  if (!isset($_GET['orderby'])) {
    ...
    else if(isset($_GET['subpage']) && $_GET['subpage'] == 'wbz404_captured'){
      $tableOptions['orderby'] = "hits";
    }
    else {
  ...
  if (!isset($_GET['order'])) {
    if ($tableOptions['orderby'] == "created" || $tableOptions['orderby'] == "lastused" || $tableOptions['orderby'] == "timestamp" || $tableOptions['orderby'] == "hits") {
...
function wbz404_getLogRecords($tableOptions) {
  ...
  $query = "select " . $logs . ".redirect_id, " . $logs . ".timestamp, " . $logs . ".remote_host, " . $logs . ".referrer, " . $logs . ".action, " . $logs . ".user_agent, " . $redirects . ".url from " . $logs;
...
function wbz404_adminPage() {
  ...
  if ($action == "updateOptions") {
    ...
    if (check_admin_referer('wbz404clearUnassigned') && is_admin()) {
      wbz404_clear_unassigned_links_from_log();
    }
  }
...
function wbz404_adminLogsPage() {
  ...
  $columns['user_agent']['title'] = "User Agent";
  $columns['user_agent']['orderby'] = "user_agent";
  $columns['user_agent']['width'] = "20%";
  $columns['refer']['title'] = "Referrer";
  $columns['refer']['orderby'] = "referrer";
  $columns['refer']['width'] = "15%";
  $columns['dest']['title'] = "Action Taken";
  $columns['dest']['orderby'] = "action";
  $columns['dest']['width'] = "15%";
...
function wbz404_AdminToolsPage() {
  ...
  require_once(WBZ404_PATH.'includes/partials/clear_unassigned_links.php');
}
