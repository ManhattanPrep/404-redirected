...
function wbz404_setupRedirect($url, $status, $type, $final_dest, $code, $disabled = 0){
  ...
  $format = array('%s','%d','%d','%s','%d','%d','%d');

  $wpdb->insert($wpdb->prefix.'wbz404_redirects', $data , $format);
...
function wbz404_logRedirectHit($id, $action){
  ...
  $data = array(
    'redirect_id' => $id,
    'timestamp' => $now,
    'remote_host' => esc_sql($_SERVER['REMOTE_ADDR']),
    'referrer' => $referer,
    'action' => $action,
  ...
  $wpdb->insert($wpdb->prefix . "wbz404_logs", $data, $format);
}
...
function wbz404_clear_unassigned_links_from_log(){
  global $wpdb;
  $query = "DELETE FROM " . $wpdb->prefix . "wbz404_logs where redirect_id = '0'";
  $wpdb->query($query);
